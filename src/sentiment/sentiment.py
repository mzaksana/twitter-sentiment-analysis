...
def features(src,dst):
	...
	for file in files:
		...
		for line in text_file.split("\n"):
			...
			line_result = [data['text']] + [feats]
			js={
				'air-line' : ntpath.basename(file).split('.')[0],
				'text': line_result[0],
				'Class': line_result[1]
			}
			print(json.dumps(js))
...
# argv -src file-path -model model.npz -positive file-path -negative file-path -stop-word file-path -emoticon path-emoticon
main()
